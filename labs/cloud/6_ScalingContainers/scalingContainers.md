...
1.  Copy the supplied server.xml file from the `{LAB_HOME}/labs/cloud/6_CaaSandDb2Warehouse` directory to the newly created server’s directory.
    `cp {LAB_HOME}/labs/cloud/6_CaaSandDb2Warehouse/server.xml {LAB_HOME)/wlp/usr/servers/labServer`
1.  Edit the server.xml file, look for the `properties.db2.jcc` xml tag. Change the elements. then **Save** the server.xml file.
    *  **serverName** with **hostname**
    *  **user** with **user name**
    *  **password** with **password**
    *  **port** should be the same
    *  **database** name should be the same
1.  Copy the EmployeeApp.war file again from the {LAB\_HOME}/labs/cloud/6\_CaaSandDb2Warehouse to the apps directory of the app server just created to complete the deployment of the EmployeeApp application in Liberty.
    `cp {LAB_HOME}/labs/cloud/6_CaaSandDb2Warehouse/EmployeeApp.war {LAB_HOME}/wlp/usr/servers/labServer/apps`
1.  Start the liberty server using the command
    `server run labServer`
1.  Your Liberty server should be up and running with the EmployeeApp application.
1.  Launch the application using the URL <http://localhost:9080/EmployeeApp>
...
1.  Initialize the IBM Cloud container service plug-in and logon to the IBM Cloud container registry service. Execute the following commands
    `bx cs init`   
     ~~~~
     bx cs init
...
1. To work with the Kubernetes commands on a Kubernetes cluster, you need the kubectl CLI. This should have been installed during section 2.
   1.  From a command line, type kubectl version –short command, it will only show the client version, as shown here, since the kubectl CLI is not yet connected to the Kubernetes cluster running in IBM Bluemix.
    ~~~~
